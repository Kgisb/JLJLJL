...
def tt__duration_to_secs(s):
    """Parse Call Duration into seconds. Accepts 'HH:MM:SS', 'H:MM:SS', 'MM:SS', 'SS' or numeric seconds."""
    if s is None:
        return _tt_np.nan
    ...
    if not ss:
        return _tt_np.nan
    # Replace possible H;MM;SS with H:MM:SS
    ss = ss.replace(";", ":")
...
def tt__parse_date(d):
    """Parse Date column robustly (DD-MM-YYYY or YYYY-MM-DD). Returns date() or NaT."""
    if d is None:
        return _tt_pd.NaT
    s = str(d).strip()
    if not s:
    ...
    try:
        return _tt_pd.to_datetime(s, errors="coerce").date()
    except:
        return _tt_pd.NaT

def tt__parse_time(t):
    """Parse Time column robustly (HH:MM[:SS]). Returns time() or NaT."""
    if t is None:
        return _tt_pd.NaT
    s = str(t).strip()
    if not s:
    ...
    try:
        ...
        return _tt_pd.NaT
    if len(parts) == 1:
        hh = parts[0]; mm = 0; ss = 0
    elif len(parts) == 2:
        hh, mm = parts; ss = 0
    else:
        hh, mm, ss = (parts + [0, 0, 0])[:3]
    try:
        return _tt_time(hh, mm, ss)
    except:
        return _tt_pd.NaT
...
def _render_marketing_talk_time(df):
    ...
    dff["_tt_dt"] = dff.apply(tt__combine_dt, axis=1)
