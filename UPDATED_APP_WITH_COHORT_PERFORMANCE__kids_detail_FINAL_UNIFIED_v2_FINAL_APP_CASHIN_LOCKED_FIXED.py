...
try:
    ...
    if _ci_is_cash_in:
        ...
        _ci_st.write("Using locked source:")
        _ci_st.code(f"Sheet ID: {_CI_DEFAULT_SHEET_ID}\nGID: {_CI_DEFAULT_GID}")
        if _ci_st.button("Reload Cash-in data", key="cashin_reload"):
            _ci_st.cache_data.clear()
        try:
            df_cashin = _ci_fetch_sheet(_CI_DEFAULT_SHEET_ID, _CI_DEFAULT_GID)
            _ci_st.success(f"Loaded Cash-in sheet with {len(df_cashin):,} rows (A1:D14).")
            _ci_st.session_state["cashin_df"] = df_cashin
        except Exception as _e:
            _ci_st.error(f"Unable to load Cash-in sheet: {type(_e).__name__}: {_e}")
            _ci_st.info("Ensure the sheet is shared for 'Anyone with the link' (Viewer).")
