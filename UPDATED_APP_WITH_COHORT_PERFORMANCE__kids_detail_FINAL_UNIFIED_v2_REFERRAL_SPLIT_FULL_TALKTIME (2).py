...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Ref_Tracker", "Referral Split", "Talk Time" ],
    }
...
def tt__duration_to_secs(s):
    ...
    Returns np.nan if truly unparseable.
    """
...
def tt__excel_serial_to_date(val):
    ...
    if f <= 0 or f > 100000:
        return pd.NaT
    try:
        return (pd.to_datetime("1899-12-30") + pd.to_timedelta(f, unit="D")).date()
    except Exception:
        return pd.NaT
...
def tt__parse_time(t):
    if isinstance(t, (int, float)) and not pd.isna(t):
        ...
        try:
            return pd.Timestamp(year=2000, month=1, day=1, hour=hh, minute=mm, second=ss).time()
        except:
            return pd.NaT
    s = str(t).strip()
    if not s or s.lower() in {"nan","none","-"}:
    ...
    for fmt in ("%H:%M:%S", "%H:%M", "%I:%M %p", "%I:%M:%S %p"):
        try:
            return pd.to_datetime(sn, format=fmt, errors="raise").time()
        except Exception:
            pass
    dt = pd.to_datetime(sn, errors="coerce")
    return dt.time() if not pd.isna(dt) else pd.NaT
...
def _render_marketing_talk_time(df):
    ...
    if up is not None:
        try:
            dff = _tt_pd.read_csv(up)
        except Exception as e:
            _tt_st.error(f"Unable to read CSV: {e}")
            return
    else:
    ...
    dff["_tt_Date"] = dff["Date"].apply(tt__parse_date) if "Date" in dff.columns else pd.NaT
    dff["_tt_Time"] = dff["Time"].apply(tt__parse_time) if "Time" in dff.columns else pd.NaT
    ...
    dt_combo = pd.to_datetime(combo, errors="coerce", dayfirst=True)
    dt_combo2 = pd.to_datetime(combo, errors="coerce", dayfirst=False)
    ...
    dff["_tt_secs"] = dff["Call Duration"].apply(tt__duration_to_secs) if "Call Duration" in dff.columns else np.nan
    dff["_tt_hour"] = dff["_tt_dt"].dt.hour
    dff["_tt_is_60"] = dff["_tt_secs"] > 60

    # Diagnostics before exclusion
    dff["_reason_dt"] = np.where(dff["_tt_dt"].isna(), "Bad Date/Time", "")
    dff["_reason_secs"] = np.where(dff["_tt_secs"].isna(), "Bad Duration", "")
