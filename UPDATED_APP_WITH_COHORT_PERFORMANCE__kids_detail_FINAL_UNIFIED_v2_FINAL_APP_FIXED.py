...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Pipeline", "Sales Activity", "Deal stage", "Original source", "Referral / No-Referral", "Lead mix", "Referral performance", "Slow Working Deals", "Activity concentration" ],
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time" ],
    }
...
# === Right-side Vertical Pill Rail (per current section) ===
try:
    import streamlit as _rr_st
    _rr_st.set_page_config(layout="wide")

    # Section → pills map inferred from your app (excluding disabled pills)
    _RR_SEC_TO_PILLS = {"Stage": ["Deals", "First Trial Scheduled", "Trial Rescheduled", "Trial Done", "Enrollments"], "Performance": ["Cash-in", "Dashboard", "MIS", "Daily Business", "Sales Tracker", "AC Wise Detail", "Leaderboard", "Quick View", "Comparison", "Pipeline", "Sales Activity", "Deal stage", "Original source", "Referral / No-Referral", "Lead mix", "Referral performance", "Slow Working Deals", "Activity concentration"], "Funnel & Movement": ["Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward"], "Insights & Forecast": ["Predictibility", "Business Projection", "Buying Propensity", "80-20", "Trend & Analysis", "Heatmap", "Bubble Explorer", "Master Graph"], "Marketing": ["Referrals", "HubSpot Deal Score tracker", "Marketing Lead Performance & Requirement", "Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time"], "Category": ["Referral", "Non-Referral"], "Metric": ["Actual MTD", "Projected Month-End"], "Bucket": ["A ≤{int(N)}d", "B >{int(N)}d"], "Status": ["Paid Referrer", "Not Paid"]}

    # Read current nav state from your app/session
    _rr_current_section = _rr_st.session_state.get("nav_master", list(_RR_SEC_TO_PILLS.keys())[0] if _RR_SEC_TO_PILLS else "Marketing")
    _rr_pills = _RR_SEC_TO_PILLS.get(_rr_current_section, [])

    # Prepare CSS for a sticky, right-edge, collapsible rail
    _rr_st.markdown("""
    <style>
      .rr-rail { position: fixed; right: 16px; top: 72px; z-index: 999; width: 280px; }
      .rr-card { background: var(--background-color, #fff); border-radius: 12px;
                 box-shadow: 0 4px 18px rgba(0,0,0,0.12); padding: 12px; }
      @media (max-width: 1200px) { .rr-rail { display: none; } } /* hide on small screens */
      .rr-radio label { display:block; padding: 4px 0; }
    </style>
    """, unsafe_allow_html=True)

    _rr_st.markdown('<div class="rr-rail"><div class="rr-card">', unsafe_allow_html=True)
    with _rr_st.expander(f"{_rr_current_section} — Pills", expanded=False):
        if not _rr_pills:
            _rr_st.info("No pills available for this section.")
        else:
            # Choose current pill; use existing view if present
            _rr_current_pill = _rr_st.session_state.get("nav_sub", _rr_pills[0])
            _rr_default_idx = _rr_pills.index(_rr_current_pill) if _rr_current_pill in _rr_pills else 0
            # Vertical radio of pills (order preserved)
            _rr_new_pill = _rr_st.radio("Pills", _rr_pills, index=_rr_default_idx, key="rr_pill_select", label_visibility="collapsed", horizontal=False)
            if _rr_new_pill != _rr_current_pill:
                _rr_st.session_state["nav_sub"] = _rr_new_pill
                try:
                    view = _rr_new_pill  # mirror global if your routers rely on it
                except Exception:
                    pass
                _rr_st.rerun()
    _rr_st.markdown('</div></div>', unsafe_allow_html=True)

except Exception as _rr_e:
    try:
        import streamlit as _rr_st
        _rr_st.warning(f"Pill Rail temporarily unavailable: {type(_rr_e).__name__}: {_rr_e}")
    except Exception:
        pass
# === /Right-side Vertical Pill Rail ===



# ===== Cash-in Google Sheet Source (non-invasive) =====
try:
    import streamlit as _ci_st
    import pandas as _ci_pd

    # Default values from user link
    _CI_DEFAULT_SHEET_ID = "1D2ItSjgiCiPRt_3uskBGqdyRSSGkby-XmzoB8tCs9Ds"
    _CI_DEFAULT_GID = "974852750"

    def _ci_sheet_csv_url(sheet_id: str, gid: str) -> str:
        return f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=csv&gid={gid}"
    @_ci_st.cache_data(show_spinner=False)
    def _ci_fetch_sheet(sheet_id: str, gid: str):
    url = _ci_sheet_csv_url(sheet_id.strip(), gid.strip())
    df = _ci_pd.read_csv(url)
    # Restrict to A1:D14 (first 14 rows, first 4 columns)
    df = df.iloc[:14, :4]
    return df

    _ci_master = _ci_st.session_state.get("nav_master", "")
    _ci_view = _ci_st.session_state.get("nav_sub", "")
    _ci_is_cash_in = str(_ci_view).strip().lower() in {"cash-in","cash in","cashin","cash in report","cash-in report"}

    if _ci_is_cash_in:
        with _ci_st.expander("Cash-in Data Source (Google Sheet)", expanded=False):
            sheet_id = _ci_st.text_input("Google Sheet ID", value=_CI_DEFAULT_SHEET_ID, key="cashin_sheet_id")
            gid = _ci_st.text_input("GID (tab id)", value=_CI_DEFAULT_GID, key="cashin_gid")
            if _ci_st.button("Reload Cash-in data", key="cashin_reload"):
                _ci_st.cache_data.clear()

            try:
                df_cashin = _ci_fetch_sheet(sheet_id, gid)
                _ci_st.success(f"Loaded Cash-in sheet with {len(df_cashin):,} rows.")
                _ci_st.session_state["cashin_df"] = df_cashin
            except Exception as _e:
                _ci_st.error(f"Unable to load Cash-in sheet: {type(_e).__name__}: {_e}")
                _ci_st.info("Ensure the sheet is shared for 'Anyone with the link' (Viewer) or that the ID/GID are correct.")
except Exception as _outer_e:
    pass
# ===== /Cash-in Google Sheet Source =====
