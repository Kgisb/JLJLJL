...
# === Right-side Pill Tray (overlay; additive and non-invasive) ===
try:
    import streamlit as _rt_st
    _rt_st.set_page_config(layout="wide")
    _RT_SEC_TO_PILLS = {"Stage": ["Deals", "First Trial Scheduled", "Trial Rescheduled", "Trial Done", "Enrollments"], "Performance": ["Cash-in", "Dashboard", "MIS", "Daily Business", "Sales Tracker", "AC Wise Detail", "Leaderboard", "Quick View", "Comparison", "Pipeline", "Sales Activity", "Deal stage", "Original source", "Referral / No-Referral", "Lead mix", "Referral performance", "Slow Working Deals", "Activity concentration"], "Funnel & Movement": ["Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward"], "Insights & Forecast": ["Predictibility", "Business Projection", "Buying Propensity", "80-20", "Trend & Analysis", "Heatmap", "Bubble Explorer", "Master Graph"], "Marketing": ["Referrals", "HubSpot Deal Score tracker", "Marketing Lead Performance & Requirement", "Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time"], "Category": ["Referral", "Non-Referral"], "Metric": ["Actual MTD", "Projected Month-End"], "Bucket": ["A ≤{int(N)}d", "B >{int(N)}d"], "Status": ["Paid Referrer", "Not Paid"]}

    # Current selection fallbacks
    _rt_current_section = _rt_st.session_state.get("nav_master", list(_RT_SEC_TO_PILLS.keys())[0] if _RT_SEC_TO_PILLS else "Marketing")
    _rt_current_pills = _RT_SEC_TO_PILLS.get(_rt_current_section, [])
    _rt_current_pill = _rt_st.session_state.get("nav_sub", (_rt_current_pills[0] if _rt_current_pills else None))

    _rt_st.markdown("""
    <style>
      .rt-tray { position: fixed; right: 16px; top: 72px; z-index: 998; width: 300px; }
      .rt-card { background: var(--background-color, #fff); border-radius: 12px; 
                  box-shadow: 0 4px 18px rgba(0,0,0,0.12); padding: 12px; }
      @media (max-width: 1200px) { .rt-tray { display: none; } } /* hide on small screens */
    </style>
    """, unsafe_allow_html=True)

    _rt_st.markdown('<div class="rt-tray"><div class="rt-card">', unsafe_allow_html=True)
    with _rt_st.expander("Pill Tray (all sections)", expanded=False):
        _rt_sections = list(_RT_SEC_TO_PILLS.keys())
        if _rt_current_section not in _rt_sections and _rt_sections:
            _rt_current_section = _rt_sections[0]
        _rt_new_section = _rt_st.selectbox("Section", _rt_sections, 
                                           index=_rt_sections.index(_rt_current_section) if _rt_sections else 0,
                                           key="rt_section_select")
        _rt_pills = _RT_SEC_TO_PILLS.get(_rt_new_section, [])
        if _rt_pills:
            _rt_default_idx = _rt_pills.index(_rt_current_pill) if _rt_current_pill in _rt_pills else 0
            _rt_new_pill = _rt_st.radio("Pills", _rt_pills, index=_rt_default_idx, key="rt_pill_select")
        else:
            _rt_new_pill = None
            _rt_st.info("No pills available for this section.")

        # Sync to existing navigation state and trigger rerun if changed
        if (_rt_new_section != _rt_current_section) or (_rt_new_pill != _rt_current_pill):
            _rt_st.session_state["nav_master"] = _rt_new_section
            _rt_st.session_state["nav_sub"] = _rt_new_pill
            try:
                master = _rt_new_section
                view = _rt_new_pill
            except Exception:
                pass
            _rt_st.rerun()
    _rt_st.markdown('</div></div>', unsafe_allow_html=True)
except Exception as _rt_e:
    try:
        import streamlit as _rt_st
        _rt_st.warning(f"Right-side Pill Tray temporarily unavailable: {type(_rt_e).__name__}: {_rt_e}")
    except Exception:
        pass
# === /Right-side Pill Tray ===
