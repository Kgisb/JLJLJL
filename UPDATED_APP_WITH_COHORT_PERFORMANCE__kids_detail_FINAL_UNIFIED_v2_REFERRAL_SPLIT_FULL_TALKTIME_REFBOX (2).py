...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Ref_Tracker", "Referral Split", "Referral Box", "Talk Time" ],
    }
...
def tt__read_csv_flexible(upfile, manual_sep=None):
    ...
    seps = [",",";","\t","|"]
    # If manual sep provided, try that first across encodings
    ...
    # Auto-infer with engine='python', sep=None
    for enc in encodings:
        for bad in ["skip"]:
...
def tt__duration_to_secs(s):
    """Robust parse of Call Duration into seconds.
    Accepts: "HH:MM:SS", "H:MM:SS", "MM:SS", "SS", "1h 2m 3s", "2m 10s", "45s",
    Excel-style "0:00:45.000", "1.02.03", semicolons, or raw numeric seconds.
    Returns _tt_np.nan if truly unparseable.
    """
...
def tt__parse_date(d):
    if isinstance(d, _tt_pd.Timestamp):
        return d.date()
    if isinstance(_tt_dt, _tt_date):
        return _tt_pd.to_datetime(d).date()
    if isinstance(d, (int, float)) and not _tt_pd.isna(d):
        dt = tt__excel_serial_to_date(d)
        return dt
    s = str(d).strip()
...
def tt__parse_time(t):
    if isinstance(t, (int, float)) and not _tt_pd.isna(t):
        secs = int(round(float(t)))
        secs = max(0, min(secs, 24*3600-1))
...
def tt__date_preset_bounds(preset, today=None):
    ...
    if preset == "Yesterday":
        y = tz_today - _tt_timedelta(days=1)
        return y, y
    if preset == "This Month":
        start = tz_today.replace(day=1)
        return start, tz_today
    if preset == "Last Month":
        first_this = tz_today.replace(day=1)
        last_month_end = first_this - _tt_timedelta(days=1)
...
def _render_marketing_talk_time(df):
    ...
    if up is not None:
        # Optional manual delimiter override
        ...
        if df_try is None:
            ...
            _tt_st.expander("Open parser debug log").write(debug_log)
            return
        dff = df_try
    else:
        # Try to reuse df if it contains the necessary columns; otherwise prompt upload
        ...
        if dff is None or not set(["Date","Time","Caller","Call Type","Country Name","Call Status","Call Duration"]).issubset(set(map(str, dff.columns))):
            _tt_st.info("Please upload the **activity feed CSV** with columns: Date, Time, Caller, Call Type, Country Name, Call Status, Call Duration.")
            return

    # Keep only required columns (silently ignore extras)
    needed = ["Date","Time","Caller","Call Type","Country Name","Call Status","Call Duration"]
    present = [c for c in needed if c in dff.columns]
    dff = dff[present].copy()
    ...
    # Combined fallback (handles AM/PM, odd separators, etc.)
    ...
    # Prefer precise combine when both parts parsed; fill remaining from combo attempts
    ...
    if _bad_count:
        import streamlit as st
        st.warning(f"Excluded {_bad_count} rows with unparseable Date/Time or Duration.")
        excl = dff.loc[bad_rows, ["Date","Time","Caller","Call Type","Country Name","Call Status","Call Duration","_reason_dt","_reason_secs"]].copy()
        st.download_button("Download excluded rows (CSV)", excl.to_csv(index=False).encode("utf-8"), "talk_time_excluded_rows.csv", "text/csv", key="tt_dl_excluded")
    # Basic data quality
    bad_rows = dff["_tt_dt"].isna() | dff["_tt_secs"].isna()
    if bad_rows.any():
        _tt_st.warning(f"Excluded {int(bad_rows.sum())} rows with unparseable Date/Time or Duration.")
    ...
    with c4:
        sel_agents = _tt_st.multiselect("Agent(s)", agents, default=agents[:10], key="tt_agents")
    with c5:
        sel_countries = _tt_st.multiselect("Country", countries, default=countries[:10], key="tt_countries")
    with c6:
        status_mode = _tt_st.selectbox("Status", ["Connected only","All statuses"], index=0, key="tt_status_mode")
    with c7:
        sel_ctype = _tt_st.multiselect("Call Type", ctypes, default=ctypes, key="tt_ctype")
    ...
    if sel_agents:
        mask &= dff["Caller"].isin(sel_agents)
    if countries and sel_countries:
        mask &= dff["Country Name"].isin(sel_countries)
    if ctypes and sel_ctype:
        mask &= dff["Call Type"].isin(sel_ctype)
    if status_mode == "Connected only" and "Call Status" in dff.columns:
        # Keep statuses that imply a connected conversation; we accept anything non-empty that has positive duration
        mask &= (dff["_tt_secs"] > 0)

    dfv = dff[mask].copy()
    if gate_mode == "> 60s only":
        dfv = dfv[dfv["_tt_is_60"]].copy()

    # KPIs
    total_secs = dfv["_tt_secs"].sum() if len(dfv) else 0
    n_calls = int(len(dfv))
    avg_secs = (total_secs / n_calls) if n_calls > 0 else 0
    pct_60 = (100.0 * dfv["_tt_is_60"].mean()) if len(dfv) else 0.0
    ...
    if gb_col not in dfv.columns or dfv[gb_col].isna().all():
        ...
        chs = dfv.groupby("_tt_hour")["_tt_secs"].sum().reset_index()
        chs.rename(columns={"_tt_hour":"Hour","_tt_secs":"Total Seconds"}, inplace=True)
        ...
        # All calls
        csrc_all = dfv.groupby([gb_col, "_tt_hour"])["_tt_secs"].sum().reset_index()
        csrc_all.rename(columns={gb_col: y_title, "_tt_hour":"Hour", "_tt_secs":"Total Seconds"}, inplace=True)
        ...
        csrc_60 = dfv60.groupby([gb_col, "_tt_hour"])["_tt_secs"].sum().reset_index()
        csrc_60.rename(columns={gb_col: y_title, "_tt_hour":"Hour", "_tt_secs":"Total Seconds"}, inplace=True)
...
# ---- Optional lightweight router hook (no-op if duplicate) ----
try:
    # If the app uses a function factory based on naming, this is not needed.
    # But if there is a central dispatcher that calls functions by explicit name,
    # you can call this to render:
    # _render_marketing_talk_time(df)
    pass
except Exception as _tt_e:
    # Do nothing if environment doesn't match
    pass
...
try:
    ...
    if _master_for_tt == "Marketing" and _view_for_tt == "Talk Time":
        # Note: This renderer can work purely off the uploaded CSV.
        # If you want to provide a default df to it, pass the main df object.
        try:
            _render_marketing_talk_time(df)
        except TypeError:
            # Some app variants call with df_f only; fall back gracefully.
...
def _rb_date_preset_bounds(preset, today=None):
    ...
    if preset == "Today":
        return tz_today, tz_today
    if preset == "Yesterday":
        y = tz_today - _rb_timedelta(days=1); return y, y
    if preset == "This Month":
        start = tz_today.replace(day=1); return start, tz_today
    if preset == "Last Month":
        first_this = tz_today.replace(day=1)
...
def _rb_find_col(df, candidates, default=None, keywords=None):
    ...
    # 2) normalized equality against candidates
    ...
    if keywords:
        ...
        for c in cols:
            ...
            if all(k in nc for k in kw):
                return c
    # 4) heuristic: pick the first candidate that "contains" a candidate token
    for name in candidates or []:
        token = norm(name)
        for c in cols:
...
def _render_marketing_referral_box(df_f, create_col, pay_col, source_col, country_col, agent_col):
    ...
    _source = _rb_find_col(df_f, [source_col, "Referral Intent Source","Referral Intent","Referral Source","Intent Source"])
    _parent_email = _rb_find_col(df_f, ["Parent Email ID","Parent Email","Email","Parent_Email_ID","Parent Email Id"])
    _ref_by_email = _rb_find_col(df_f,
        ["Referred Parent Referred By Email","Parent Referred By Email","Referrer Email","Referred By Email","Parent Email Referred By","Parent Referred By","Referral Referred By Email","Referrer Parent Email","Referred Parent By Email","Ref By Email","Ref By Parent Email"],
        keywords=["referred","by","email"]
    )
    ...
    missing = [n for n,v in {
        ...
        "Referral Intent Source": _source,
        "Parent Email ID": _parent_email,
        "Referred Parent Referred By Email": (_ref_by_email if _ref_by_email is not None else "PENDING")
    }.items() if v is None]
    if missing:
        _rb_st.error("Missing required columns: " + ", ".join(missing)); return

    # UI fallback: if referrer email column couldn't be auto-detected, ask the user
    if _ref_by_email is None:
        _rb_st.warning("Referrer Email column was not detected automatically. Please select it below.")
        email_like_cols = [c for c in df.columns if "mail" in c.lower() or "email" in c.lower() or "referred" in c.lower()]
    ...
    if scope == "Self-Generated":
        sg_vals = df[_source].astype(str).str.strip().str.lower()
        mask &= sg_vals.str.replace("-", " ", regex=False).isin(["self generated","self generated referral","self generated lead","self generateds","self generateds referral","self generateds lead"])
        # also accept 'self-generated' direct
        mask |= df[_source].astype(str).str.strip().str.lower().eq("self-generated")
    # Date window
    if _create:
    ...
    # Build referrer index from the full dataset (not just base)
    ref = df[[*_rb_pd.unique([_parent_email, _pay])]].copy()
    ref = ref.rename(columns={_parent_email:"_rb_parent_email", _pay:"_rb_pay"})
    ref["_rb_paid"] = ref["_rb_pay"].notna()
    grp = ref.groupby("_rb_parent_email", dropna=True).agg(
        _rb_first_enroll=("._rb_pay".replace(".", ""), "min") if False else ("_rb_pay","min"),
        _rb_paid_ever=("_rb_paid","max"),
    ).reset_index()
    # Left-join base referrals to referrer index via referrer email
    ...
    sum_df = _rb_pd.DataFrame({
        "Status":["Paid Referrer","Not Paid"],
        "Referrals":[paid_count, not_paid]
    })
    ...
    det = base.groupby(["_rb_referrer","_rb_paid_ref"], dropna=False).agg(
        Referrals=("Parent Email ID" if "Parent Email ID" in base.columns else _parent_email, "count"),
    ...
    rows = base[view_cols + derived].copy()
    rows = rows.rename(columns={
        ...
        _source:"Referral Intent Source",
...
# --- Router: Marketing → Referral Box ---
try:
    _master_for_rb = master if 'master' in globals() else st.session_state.get('nav_master', '')
    _view_for_rb = view if 'view' in globals() else st.session_state.get('nav_sub', '')
    if _master_for_rb == "Marketing" and _view_for_rb == "Referral Box":
        _render_marketing_referral_box(
            df_f=df_f if 'df_f' in globals() else df,
            create_col=create_col if 'create_col' in globals() else "Create Date",
            pay_col=pay_col if 'pay_col' in globals() else "Payment Received Date",
            source_col=source_col if 'source_col' in globals() else "Referral Intent Source",
            country_col=country_col if 'country_col' in globals() else "Country Name",
            agent_col=agent_col if 'agent_col' in globals() else "Academic Counselor"
        )
except Exception as _e_rb:
    import streamlit as st
    st.error(f"Referral Box failed: {type(_e_rb).__name__}: {_e_rb}")
# --- /Router: Marketing → Referral Box ---
